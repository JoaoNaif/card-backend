...
describe('FetchCharacterRosterUserUseCase', () => {
  beforeEach(() => {
    ...
    sut = new FetchCharacterRosterUserUseCase(characterRepository, userRepository)
  })

  it('should return the characters belonging to the user', async () => {
    const user = makeUser()
    await userRepository.create(user)

    const userId = user.id.toString()

    const char1 = makeCharacter({ userId })
    const char2 = makeCharacter({ userId })
  ...
  it('should not return characters from other users', async () => {
    ...
    const char1 = makeCharacter({ userId: user1.id.toString() })
    const char2 = makeCharacter({ userId: user2.id.toString() })
    await characterRepository.create(char1)
    await characterRepository.create(char2)
