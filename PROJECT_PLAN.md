...
3.  _Observação:_ A modelagem exata de armazenamento das características (se array nativo `String[]` do Postgres ou tabela separada) e as mudanças estruturais de habilidades (`Skill.cost` para `Int`) serão definidas quando iniciarmos esta etapa.
...
Trait → Power → Skill → Character → CharacterSkill (via use cases de level up)
